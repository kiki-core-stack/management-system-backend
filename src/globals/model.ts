...
setReadonlyConstantToGlobalThis<typeof getModelDocumentByRouteIdAndDelete>(
    'getModelDocumentByRouteIdAndDelete',
    async (ctx, model, options, beforeDelete) => {
...
setReadonlyConstantToGlobalThis<typeof getModelDocumentByRouteIdAndUpdateBooleanField>(
    'getModelDocumentByRouteIdAndUpdateBooleanField',
    async (ctx, model, allowedFields, options, beforeUpdate) => {
        const document = await model.findByRouteIdOrThrowNotFoundError(ctx, undefined, null, options);
        const {
            field,
            value,
